...
\begin{itemize}
  ...
  \item We implement a comprehensive safety layer (MOSAIC) that achieves 86.4\% threat detection rate with 3.2\% false positive rate, addressing both execution safety and budget constraints.
  \item We introduce INTENT, a budget-aware planning mechanism that reduces operational costs by 34\% while maintaining task completion quality.
...
We tested AgentOS on 100 multi-step tasks across three domains: software development, data analysis, and research automation. Tasks were categorized by complexity: 2-4 steps (simple), 5-9 steps (moderate), and 10+ steps (complex).

\begin{table}[htbp]
\centering
\caption{Task Completion Rates by Task Complexity}
\begin{tabular}{lccc}
\toprule
Method & Simple (2-4) & Moderate (5-9) & Complex (10+) \\
\midrule
Single-pass decomposition & 89\% & 61\% & 34\% \\
ROMA only & 91\% & 74\% & 51\% \\
ROMA + HTAA & \textbf{94\%} & \textbf{84\%} & \textbf{67\%} \\
ROMA + HTAA + REDEREF & \textbf{96\%} & \textbf{87\%} & \textbf{72\%} \\
\bottomrule
\end{tabular}
\label{tab:completion}
\end{table}

Table~\ref{tab:completion} demonstrates that combining ROMA's recursive decomposition with HTAA's tool grouping achieves substantial improvements, with further gains from reflective learning (REDEREF).

\subsection{Safety Performance}

\begin{table}[htbp]
\centering
\caption{Safety Evaluation: MOSAIC Threat Detection}
\begin{tabular}{lcccc}
\toprule
Threat Category & Blocked & Allowed & Total & Detection Rate \\
\midrule
Path traversal & 43 & 9 & 52 & 82.7\% \\
Command injection & 29 & 2 & 31 & 93.5\% \\
Unauthorized network & 38 & 3 & 41 & 92.7\% \\
Budget violation & 17 & 6 & 23 & 73.9\% \\
\midrule
Total & 127 & 20 & 147 & 86.4\% \\
\bottomrule
\end{tabular}
\label{tab:safety}
\end{table}

Table~\ref{tab:safety} shows that MOSAIC achieved 86.4\% overall block rate. False positive rate (benign operations incorrectly blocked) was measured at 3.2\% across 500 benign task evaluations.

\subsection{Cost Efficiency}

INTENT budget management reduced average task cost by 34\% compared to unconstrained execution ($0.89$ average cost vs $\$1.35$ baseline). This reduction came from early termination of low-value task branches when budget constraints were reached, without sacrificing successful task completion.

% 6. Conclusion and Future Work
\section{Conclusion}

We presented AgentOS, a comprehensive pipeline architecture for autonomous task completion in LLM-based agents. Through systematic integration of ROMA, HTAA, ToolTree, MOSAIC, and REDEREF, AgentOS achieves high task completion rates across complexity levels while maintaining strong safety guarantees and cost efficiency.

Future work includes: (1) formal verification of safety properties using the framework of \citet{doshi2026safe}, (2) multi-agent collaboration through ROMA-style agent coordination, (3) comprehensive benchmarking against established agent frameworks including AutoGPT and BabyAGI, and (4) integration with additional LLM providers and tool registries.
